...
void HOUSE::setParent(HOUSE* parenthouse)
{
	...
	{
		mc->parent=NULL;
		mc->scene->parent=NULL;
		mc->tmp_scene->parent=NULL;
	}
}
//#################################################################//
...
*/
void HOUSE::shape(int type,int mode,int x1,int y1,int z1,int x2,int y2,int z2,char* src)
...
*/
 shape_raw(type,mode,x1,y1,z1,x2,y2,z2,0,src);
 shape_raw(type,mode,x1,y1,z1,x2,y2,z2,1,src);
 shape_raw(type,mode,x1,y1,z1,x2,y2,z2,2,src);
...
void HOUSE::shape_raw(int type,int mode,int x1,int y1,int z1,int x2,int y2,int z2,int block)
{
 ...
 for(x=x1;x!=x2;x++)
 {
	 ...
	 for(y=y1;y!=y2;y++)
	 {
		...
		offset=x+(y+z1*sy)*sx-offset_add;;
...
*/
// after coordinate transformation
void HOUSE::shape_raw(int type,int mode,int x1,int y1,int z1,int x2,int y2,int z2,int block,char* src)
//void HOUSE::shape_raw(int type,int mode,int x1,int y1,int z1,int x2,int y2,int z2,int block)
{
 if(x2<clipstart[block].x)return;
 if(y2<clipstart[block].y)return;
 ...
 int oy = 0;
 int oz = 0;
 unsigned int dxo = (x2-x1)*scale/4; dxo/=8;
 unsigned int dyo = (y2-y1)*scale/4;
 unsigned int dzo = (z2-z1)*scale/4;
 unsigned int dtotal = dxo*dyo*dzo;

 if(x1<clipstart[block].x){ ox = clipstart[block].x-x1; x1=clipstart[block].x; }
 if(y1<clipstart[block].y){ oy = clipstart[block].y-y1; y1=clipstart[block].y; }
 if(z1<clipstart[block].z){ oz = clipstart[block].z-z1; z1=clipstart[block].z; }
...
/*		  if(
		 ...
		 if(inside)
		 {
		  ...
		  if(type==5)//src
		  {
			  int xos = (scale/4)*(ox+x-x1);
			  int xan = 1 << (xos & 7);
				  xos/=8;

			  unsigned int src_ofs = 
				  unsigned int(xos)+
				  unsigned int((scale/4)*(oy+y-y1))*dxo+
				  unsigned int((scale/4)*(oz+z-z1))*dxo*dyo;

				src_ofs %= dtotal;
			  //if ( src_ofs > 0 )
			  //if ( src_ofs < dtotal)
				data[x+(y+z*sy)*sx]=src[src_ofs]&xan;

		  }
		  if(type==4)//if(src)
		  {

			  int xofi = (((4*scale)/4)*(ox+x-x1));//&63;
			  int yofi = ((((4*scale)/4)*(oy+y-y1))+0);//&63;
			  int zofi = (((4*scale)/4)*(oz+z-z1));//&63;
//			  float xof = float((4*scale/4)*(ox+x-x1))/20;
//			  float yof = float((4*scale/4)*(oy+y-y1))/20;
//			  float zof = float((4*scale/4)*(oz+z-z1))/20;
//			  int a = -19*3+(abs(xofi-31)+abs(yofi-31)+abs(zofi-31));
			  int a = (xofi^(4000-yofi)^zofi)-3500;
//			  float a=sin(xof)+sin(yof)+sin(zof);
			  /*
			  for(float t=0;t<20;t+=4)
			  {
				  a+=sin(xof*t+a/2+1+t*4)+sin(yof*t-a/5+2+t*5)+sin(zof*t+a/7-3);
			  }
			  a+=yof/100-2;//cos((xof+yof+3)*t*2+4)*sin((yof+zof)*2*t+1);
			  a=*/
